...
export default function ServiceLayout({ data, onBook }) {
    if (!data || !data.sections) return null;

    // Helper to extract section data by type
    ...
    const overview = getSection('service_overview');
    const scope = getSection('service_scope');
    const expertsConfig = getSection('service_experts');
    const cta = getSection('cta');

    // Filter experts
    // Logic: if selectedDoctorIds exists (specific selection), use that.
    // If not, use roleKeyword.
    const keyword = expertsConfig.roleKeyword || '';
    const selectedIds = expertsConfig.selectedDoctorIds || []; // Array of doc IDs

    let experts = [];

    if (selectedIds && selectedIds.length > 0) {
        experts = doctors.filter(doc => selectedIds.includes(doc.id));
    } else if (keyword) {
        experts = doctors.filter(doc => doc.role.toLowerCase().includes(keyword.toLowerCase()));
    }
                    ...
                    <div className="grid lg:grid-cols-2 gap-12 md:gap-20 items-center">
                        ...
                        <div className="order-2 lg:order-1">
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <span className="text-brand-blue font-bold tracking-widest uppercase text-sm">{hero.tagline || 'Excellence'}</span>
                            </div>

                            <h1 className="text-4xl md:text-5xl lg:text-5xl font-bold text-brand-dark leading-[1.1] mb-6 tracking-tight">
                                {hero.title} <br />
                                <span className="text-gray-400 font-light">{hero.subtitle}</span>
                            </h1>

                            <p className="text-lg md:text-xl text-gray-500 leading-relaxed mb-10 max-w-lg">
                                Experience expert care designed for long-term vision health.
                            </p>
            ...
            <section className="px-6 max-w-[90rem] mx-auto py-16">
                <div className="flex flex-col lg:flex-row gap-10 items-center max-w-7xl mx-auto">
                    ...
                    <div className="lg:w-3/5">
                        <span className="text-brand-blue font-bold tracking-widest uppercase text-xs mb-2 block">{overview.title}</span>
                        <p className="text-base font-bold text-brand-dark mb-4 leading-relaxed">{overview.overviewText}</p>

                        <div
                            className="text-gray-600 leading-relaxed max-w-none prose prose-p:text-gray-600 prose-headings:text-brand-dark mb-6 text-sm"
                            dangerouslySetInnerHTML={{ __html: overview.longDesc }}
                        />
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row gap-12 items-center">
                        <div className="md:w-1/2">
                            ...
                            <h2 className="text-3xl md:text-4xl font-bold text-brand-dark mb-8">{scope.title}</h2>
                            <div className="grid gap-6">
                                {scope.points && scope.points.map((point, idx) => (
            ...
            {experts.length > 0 && (
                <section className="py-24 px-6 max-w-7xl mx-auto">
                    <div className="text-center mb-12">
                        ...
                        <h2 className="text-3xl md:text-4xl font-bold text-brand-dark mt-2">{expertsConfig.title || 'Meet Your Care Team'}</h2>
                    </div>
                ...
                <div className="max-w-7xl mx-auto bg-brand-dark rounded-[3rem] p-8 md:p-16 text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10 max-w-2xl mx-auto">
                        <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">
                            {cta.title || 'Ready to restore your vision?'}
                        </h2>
                        <p className="text-gray-300 text-lg mb-8">
                            {cta.subtitle || 'Take the first step towards a clearer tomorrow.'}
                        </p>
                        <button
                            onClick={() => onBook()}
                        ...
                        >
                            <Calendar size={20} />
                            {cta.buttonText || 'Book Appointment'}
                        </button>
                    </div>
                </div>
            </section>
